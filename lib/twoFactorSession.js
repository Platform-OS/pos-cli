...
import { isPortalOutage, portalOutageMessage } from './utils/partnerPortal.js';
import { TwoFactorError, withTwoFactor } from './utils/twoFactor.js';
...
//
// Its own function for the one failure that is not about the code at all: a Portal that is
// being deployed answers 5xx, or nothing, and that error used to reach reportCommandError
// as an ordinary HTTP failure and be reported as "You are unauthorized to do this
// operation. Check if your Token/URL are correct" -- the wrong diagnosis, and advice that
// costs an operator a working token. Re-raised as a TwoFactorError so reportCommandError
// prints the explanation as written instead of handing it to ServerError.
const stepUp = async ({ portalUrl, instanceUrl, token, email, otpCode, interactive }) => {
  try {
    return await withTwoFactor(
    ...
    if (!isPortalOutage(error)) throw error;

    throw new TwoFactorError(portalOutageMessage(normalize(portalUrl || Portal.url()), error));
  }
};

// Trades a code for a session and stores it against the environment it belongs to.
const mintSession = async ({ portalUrl, instanceUrl, token, email, otpCode, interactive }) => {
  const response = await stepUp({ portalUrl, instanceUrl, token, email, otpCode, interactive });
 ...
 * knows the cases apart — a TwoFactorError carries its own multi-line actionable text,
 * which now includes a Portal that answers 5xx or refuses the connection (stepUp turns
 * those into one), a network failure elsewhere goes to ServerError, and anything else is
 * named by the prefix — and each of them exits 1, which is what the unhandled rejection
 * did anyway, only legibly.
 */

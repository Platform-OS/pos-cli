...
// Files are attached as Blobs opened from disk, never read into a Buffer. fetch sends a Buffer
// as one chunk, and undici's 300s headersTimeout is refreshed only per chunk the socket accepts,
// so a buffered upload slower than that is cut off mid-send. A file-backed Blob streams in pieces
// and the timeout ends only a transfer that has stalled (test/unit/uploadStreaming.test.js).
const buildFormData = async (formData) => {
  const form = new FormData();
  for (const [key, value] of Object.entries(formData)) {
    if (value && typeof value === 'object' && value.path) {
      form.append(key, await fs.openAsBlob(value.path), path.basename(value.path));
...
const isRawBody = (value) =>
  value instanceof Blob || Buffer.isBuffer(value) || value instanceof Uint8Array || value instanceof ArrayBuffer;
...
const apiRequest = async ({ method = 'GET', uri, body, headers = {}, formData, json = true, forever, signal, timeout = null }) => {
  ...
  if (formData) {
    ...
    fetchOptions.body = formData instanceof FormData ? formData : await buildFormData(formData);

...
const apiRequest = async ({ method = 'GET', uri, body, headers = {}, formData, json = true, forever, signal, timeout = null }) => {
  ...
  // that gets a dispatcher; see lib/requestCeiling.js.
  const upload = carriesAFile({ formData, body });
  if (upload) fetchOptions.dispatcher = await uploadDispatcher();
  ...
  try {
    ...
    if (hitTheCeiling(e)) throw timedOutError(uri, upload ? UPLOAD_HEADERS_TIMEOUT_MS : undefined, e);
    throw e;
  } finally {
    deadline?.clear();
  }

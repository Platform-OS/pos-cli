 ...
 *
 * Setting none does not mean there is none. Node's HTTP client caps every request at 300s,
 * counted over the whole send, which used to fail the twenty-minute upload above after five
 * minutes with nothing but `fetch failed`. `apiRequest` recognises a body of bytes as an upload
 * and raises that ceiling for it -- see lib/requestCeiling.js, which is also where the bound a
 * stalled transfer eventually hits now lives.
 */

 ...
 */
// Every spinner this module has handed out. A spinner repaints its line on a timer, so
...
//
// Which of them are actually drawing is ora's own `isSpinning`, so there is nothing to
// keep in step here: no wrapping of start/stop, and no assumption about which of
// succeed/fail/warn routes through which. It is also the more accurate answer -- off a
// TTY, start() prints one line and never begins repainting, and such a spinner must not
// be "resumed" into printing it twice.
...
const pauseActiveSpinners = () => {
  const paused = [...created].filter(instance => instance.isSpinning);

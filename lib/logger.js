...
const logger = {
  Error: (message, opts) => {
    ...
    // Print to stderr synchronously so error message is always visible
    console.error(msg);

    // Call the async logger (may be slow to load)
    callInstance('Error', [msg]).catch(() => {});
  ...
  Log: (message, opts = {}) => callInstance('Log', [message]),
  ...
  programHelp: () => { program.outputHelp({error: true}); process.exit(1);},
};

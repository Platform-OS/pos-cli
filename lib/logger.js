...
const instance = (!!process.env.NO_COLOR || !!process.env.CI) ? require('./logger/simple.js') : require('./logger/rainbow.js');

const icon = path.resolve(__dirname, '../lib/pos-logo.png');
const showNotification = message => notifier.notify({ title: 'Error', message, icon });

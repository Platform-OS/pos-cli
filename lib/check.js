...
const checkThemeCheck = async () => {
  try {
    const themeCheck = await import('@platformos/theme-check-node');
    return themeCheck;
  } catch (error) {
    await logger.Error(
      'The @platformos/theme-check-node package is not installed.\n' +
      'Install it with: npm install @platformos/theme-check-node'
    );
  }
};
...
const formatOffense = (offense, basePath = null) => {
  ...
  const filePath = basePath ? path.relative(basePath, absolutePath) : absolutePath;
...
/**
 * Resolve .platformos-check.yml config path from the check root.
 * The upstream findConfigPath only looks for .theme-check.yml,
 * so we resolve it explicitly here.
 */
const resolveConfigPath = (checkPath) => {
...
const run = async (opts) => {
  ...
  let spinner;
  let theme;
  const configPath = resolveConfigPath(checkPath);
  ...
  try {
    ...
    let fileCount = 0;
    const result = await themeCheck.themeCheckRun(checkPath, configPath, (message) => {
      if (spinner && message) {

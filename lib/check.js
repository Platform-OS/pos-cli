...
const run = async (opts) => {
  ...
  try {
    ...
    if (autoFix && offenses.length > 0) {
      ...
      if (spinner) {
        spinner.text = 'Backfilling documentation...';
      }
      await themeCheck.runBackfillDocsCLI([checkPath, '--verbose', '--required']);
      ...
      if (spinner) {
        spinner.text = 'Backfilling documentation (dry run)...';
      }
      await themeCheck.runBackfillDocsCLI([checkPath, '--verbose', '--required', '--dry-run']);
    }

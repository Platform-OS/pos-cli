...
search = {
  ...
  printLogs(response, report) {
    console.table(
      Object.entries(response.hits).map((r) => {
        r[1]['date'] = this.toDate(r[1]._timestamp)
        r[1]['message'] = r[1].message.slice(0, 80)
        r[1]['type'] = r[1].type.slice(0, 50)
        return r[1]
      }),
      ["date", "type", "message"]
    )

...
// a domain applied with --no-wait needs a fresh fetch instead. Shared by dns migrate
// and dns import so both compute cutover instructions the same way (TASK-1.23).
const collectAppliedTargetStatuses = async (client, targetUuid, results) => {
  const targetStatuses = [];
  for (const result of results.filter(entry => entry.status === 'applied')) {
    targetStatuses.push(
      result.domainStatus ||
      await client.getDomain(result.domainName, targetUuid).catch(() => null)
    );
  }

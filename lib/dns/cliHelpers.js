...
// Absent reads as `undefined` (commander 15 stopped implicitly defaulting a `--no-x` flag to
// `true` when its positive `--x` counterpart is also defined), and as `true` on commander 14 —
// both are handled below, so the option keeps its meaning across either version.

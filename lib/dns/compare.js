...
// stores them short. recordKey() normalizes, or the same live record keys differently
// on each side and reports as missing on both.
const indexLiveRecords = (records, domain) => {
  const index = new Map();
  for (const record of records || []) {
    if (!record || typeof record !== 'object') continue;
    index.set(recordKey(record, domain), record);
  }
  return index;
};
...
const intentFor = (domain, name, transform, dropValuePatterns) => {
  if (!transform) {
    return { index: indexIntentRecords(domain?.attributes?.config?.extra_dns_records, name, { normalizeValues: false }), errors: [] };
  }
  const plan = transformDomain(domain, { targetInstanceUuid: 'compare', dropValuePatterns });
  // plan.kept stays valid even when the transform errored (payload null): it holds every
  // record that passed, with the www-redirect drop and --drop-value patterns applied.
  // Falling back to the raw records instead would compare asymmetrically whenever only
  // one side has a transform error; the errors surface as CRITICAL on this domain.
  return { index: indexIntentRecords(plan.kept, name, { normalizeValues: true }), errors: plan.errors };
};
...
const compareDomain = (sourceDomain, targetDomain, name, { transform, ignoreStatus, dropValuePatterns }) => {
  ...
  const sourceIntent = intentFor(sourceDomain, name, transform, dropValuePatterns);
  const targetIntent = intentFor(targetDomain, name, transform, dropValuePatterns);
  for (const error of sourceIntent.errors) critical.push(`source transform: ${error}`);
  for (const error of targetIntent.errors) critical.push(`target transform: ${error}`);
  const sourceRecords = sourceIntent.index;
  const targetRecords = targetIntent.index;
  ...
  if (verificationStripped) {
    ...
    for (const key of new Set([...sourceVerification.keys(), ...targetVerification.keys()])) {
...
const indexDomains = (domains) => {
  ...
  for (const domain of domains || []) {
    // Lowercase the key: a legacy source may report 'Example.com' while the target
    // stores 'example.com' — distinct keys would report one real domain as missing
    // on BOTH sides. Every other name comparison in this module is case-insensitive.
...
const compareInstance = (sourceDomains, targetDomains, { transform = true, ignoreStatus = false, dropValuePatterns = [] } = {}) => {
  ...
  for (const name of [...new Set([...sourceIndex.keys(), ...targetIndex.keys()])].sort()) {
    ...
    results.push(result);
    totals[LEVEL_TOTALS_KEY[result.level]] += 1;
  }

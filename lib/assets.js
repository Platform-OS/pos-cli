...
// From the Gateway already addressing this instance, so two uploads running at once cannot read
// each other's credentials. `presignUrl` still falls back to MARKETPLACE_* when given nothing.
const presignAuthFor = gateway => ({ url: gateway.url, token: gateway.token });
...
const canPresignAssetUpload = async gateway => {
  ...
  try {
    await presignDirectory(`instances/${instance.id}/assets`, presignAuthFor(gateway));
    return true;
  } catch (e) {
...
// `workDir` holds the assets zip and the manifest. The MCP server passes one per deploy: there an
// upload outlives the call that started it, so a shared path is packed over while still uploading.
const deployAssets = async (gateway, { releaseId, workDir = './tmp' } = {}) => {
  logger.Debug('Generating and uploading new assets manifest...');
  const assetsArchiveName = path.join(workDir, 'assets.zip');
  const instance = await gateway.getInstance();
  ...
  try {
    ...
    const data = await presignUrl(remoteAssetsArchiveName, assetsArchiveName, presignAuthFor(gateway));
    logger.Debug(data);
    ...
    files.writeJSON(path.join(workDir, 'assets_manifest.json'), manifest);

...
const runSingleTest = async (gateway, name) => {
  ...
  try {
    ...
    await logger.Error(`Failed to execute test: ${error.message}`, { exit: false });
    return false;
  }
};
    ...
    gateway.testRunAsync()
      ...
      .catch(async error => {
        if (ServerError.isNetworkError(error)) {
          ...
          await logger.Error(`Failed to start test execution: ${error.message}`, { exit: false });
        }
        finish(false);
      });
  });
...
Then re-run the command.`, { exit: false });
    ...
    if (ServerError.isNetworkError(error)) {
      ...
      await logger.Error(`Failed to check installed modules: ${error.message}`, { exit: false });
    }
    return false;
  }
};

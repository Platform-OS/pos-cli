...
class TestLogStream extends EventEmitter {
  ...
  processLogMessage(row) {
    ...
    // Check if this is a test path log
    ...
    if (this.testName) {
      ...
      // Show all logs after test started and before completion (including user logs)
      if (this.testStarted && !this.completed) {
        this.emit('testLog', row, logType === this.testName);
      }

      // Handle case where test framework doesn't emit "Starting unit tests" log
      // If we see a test path log with matching error_type, treat it as test start

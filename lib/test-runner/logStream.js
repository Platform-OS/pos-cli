...
class TestLogStream extends EventEmitter {
  ...
  isPlainTextSummary(message) {
    // Detect plain text summary format: "Assertions: N. Failed: N. Time: Nms"
    return /Assertions:\s*\d+\.\s*Failed:\s*\d+\.\s*Time:\s*\d+ms/i.test(message);
  }

  parsePlainTextSummary(message) {
    const match = message.match(/Assertions:\s*(\d+)\.\s*Failed:\s*(\d+)\.\s*Time:\s*(\d+)ms/i);
    if (!match) return null;

    const assertions = parseInt(match[1], 10);
  ...
  processLogMessage(row) {
    ...
    if (this.testName) {
      ...
      // Check for plain text summary format
      if (!this.completed && this.isPlainTextSummary(fullMessage)) {
        const testResults = this.parsePlainTextSummary(fullMessage);
        if (testResults) {
          this.completed = true;
          this.emit('testCompleted', testResults);
          this.stop();

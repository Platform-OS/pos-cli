...
const fetchSettings = (environment, program) => {
  ...
  if (environment) {
    logger.Error(`No settings for ${environment} environment, please see pos-cli env add`, { exit: false });
  } else {
    logger.Error('No environment specified, please pass environment for a command `pos-cli <command> [environment]`', { exit: false });
  }
  program.help();
};

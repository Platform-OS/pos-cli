...
// Own properties only: an environment named `constructor` or `toString` would otherwise resolve
// to something from Object.prototype instead of to nothing. The MCP server passes a name chosen
// by a model, and a CLI user can mistype one just as easily.
const settingsFromDotPos = (env) => {
  const config = files.getConfig();
  return config && Object.hasOwn(config, env) ? config[env] : undefined;
};

...
const storeEnvironment = settings => {
  ...
  // The same path files.getConfig() reads below. Without CONFIG_FILE_PATH here the two
  // disagree: the existing environments would be read out of the custom file and written
  // into ./.pos, leaving the file the operator named untouched and copying every stored
  // token into a second place.
  ...
  writeFileOwnerOnly(configPath, JSON.stringify(newSettings, null, 2));

...
const _readJSON = filePath => {
  if (fs.existsSync(filePath)) {
    logger.Debug(`[_readJSON] File exist: ${filePath}. Parsing.`);
    ...
    try {
      ...
      logger.Error(`Error thrown by the parser: ${error.message}`, { exit: false });
    }
  } else {
    logger.Debug(`[_readJSON] Invalid JSON: ${filePath}`);
    return {};
  }
};

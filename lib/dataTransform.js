...
const askForSlot = fileName => {
  const params = { fileName: fileName, contentLength: fs.statSync(fileName)['size'], contentType: mime.getType(fileName) };
  return new Promise((resolve, reject) => {
    request.get(
      {
...
async function processModel(model) {
  ...
  for (let itemsField of fields) {
    const items = await Promise.all(
      (model[itemsField] || []).map(item => {
        const filename = filenameForUrl(item.url);
        return download(item.url, filename)
          .then(filename => askForSlot(filename))
          .then(newUrl => uploadFile(filename, newUrl))

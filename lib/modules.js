...
const createArchive = async (moduleName) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (fs.existsSync(moduleConfigFileName)){
      releaseArchive.glob(['**/**', moduleConfigFileName], { ignore: ['**/node_modules/**'] }, { prefix: moduleName });
    } else {
      releaseArchive.glob(['**/**', moduleConfigFileName], { ignore: ['*node_modules'], cwd: `${process.cwd()}/modules/${moduleName}` }, { prefix: moduleName });
      const filePath = await moduleConfigFilePath();

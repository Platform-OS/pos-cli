...
const createArchive = async (moduleName) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (fs.existsSync(moduleConfigFileName)){
      releaseArchive.glob(['**/{private,public}/**', moduleConfigFileName], {}, { prefix: moduleName });
    } else {
      const directories = await glob(`modules/${moduleName}/{private,public}`, { onlyDirectories: true });
      directories.forEach(directory =>
        releaseArchive.directory(directory, `${moduleName}/${directory.split('/').pop()}`)
      );
      const filePath = await moduleConfigFilePath();

...
const portalAuthToken = async (email, password, otpCode) => {
  try {
    ...
    // Without this branch a 2FA account would exit(1) with nothing printed at all, since
    // a TwoFactorError is not one of ServerError's network errors.
    if (e.name === 'TwoFactorError')
      await logger.Error(e.message, { hideTimestamp: true });

...
const directoriesToIgnore = ['!.git', '!*modules'];
...
const initializeConnection = () => {
  return new Promise((resolve, reject) => {
    try {
      var serviceUrl = process.env.MARKETPLACE_URL;

      if (serviceUrl.includes('lvh.me')) {
        // Unfortunate hack for local environment, on
        // staging/production load balancer handles having more than
        // one service on the same port
        serviceUrl = serviceUrl.replace('3000', '8090');
      } else {
        // We have one deploy-service per environent hosted at deploy-service.*
        serviceUrl = serviceUrl.replace(/\/([^.]*)\./, '/deploy-service.');
      }

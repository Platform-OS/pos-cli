 ...
 * instead (Api::BaseController#require_partner_portal_available). This module is the one
 * place that recognises it, so every client path reads it the same way.
 */
...
const isPortalOutage = (error) =>
  ...
  TRANSIENT_STATUSES.includes(error?.statusCode);

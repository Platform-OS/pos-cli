...
// The Partner Portal names a two-factor failure in the 401 body precisely so a client can
// pick its next move instead of reporting the password as wrong. Every other failure -- a
// wrong password above all -- stays a bodiless 401.
...
// The portal locks an account for 15 minutes after 5 wrong codes. That budget is per
// account and is shared with every other surface, the web UI included, so stopping at 3
// leaves the operator attempts to spend elsewhere rather than locking them out of the
// portal over a mistyped digit here.
...
// Matched on the name rather than with `instanceof`: an error raised inside a Gateway can
// cross a duplicate module instance (the MCP server loads this code in-process), where two
// TwoFactorError classes exist and `instanceof` would silently answer false. Every place
// that has to recognise one asks here, so the rule is written once.
const isTwoFactorError = (error) => error?.name === 'TwoFactorError';
...
const withTwoFactor = async (run, { otpCode, interactive, unattendedHint, prelude = PASSWORD_PRELUDE } = {}) => {
  ...
  // look like the command had hung with no explanation. Paused before the prelude, not
  // just before the prompt: the prelude is the part that names the instance, the portal
  // and the account, and painting over that leaves a bare prompt with nothing to confirm
  // the target of.

...
// A log row id as the instance writes it: a microsecond epoch, `"1790008926.7639065"`, and a
// string in every row it sends. It was declared `integer`, which the admin Logs page cannot
// satisfy — it polls with `logs.at(-1).id`, so the second poll after any row arrived was a 400
// and the tail stopped advancing. Truncating is not a fix either: `last_id` is a strict
// greater-than, so a cursor without its fraction re-delivers every row from the same second.
//
// The pattern, not the type, is what keeps a caller from appending their own query parameters —
// and `Gateway.logs` encodes the value as well, which is where that guarantee really belongs.
//
// `default: '0'` matters: 0 is the "from the beginning" sentinel logStream.js already uses, so a
// first poll that sends no cursor resolves to it instead of interpolating the string "undefined"
// into the upstream URL, which is what happened before this schema existed.
//
// Exported because `logs-fetch` (mcp-min/logs/fetch.js) publishes the same shape: the cursor has
// one type wherever it travels, which is what this comment claimed while the two disagreed.

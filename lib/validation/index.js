...
/**
 * Runtime input validation, backed by Ajv.
 *
...
// Two modes, because the two input shapes need different treatment:
//
...
const MODES = {
  strict: {},
  coercing: { coerceTypes: true, useDefaults: true }
};

// Two dialects. The GUI schemas are draft-07. MCP tool schemas are 2020-12: MCP 2026-07-28
// assigns that dialect to any schema published without `$schema`, which is how every tool
// schema goes out in tools/list, so that is the dialect they are enforced in.
const DIALECTS = {
  'draft-07': Ajv,
  '2020-12': Ajv2020
};
...
const instanceFor = (mode, dialect) => {
  if (!Object.hasOwn(MODES, mode)) {
    throw new RangeError(`Unknown validation mode: ${mode} (expected ${Object.keys(MODES).join(' or ')})`);
  }
  if (!Object.hasOwn(DIALECTS, dialect)) {
    throw new RangeError(`Unknown JSON Schema dialect: ${dialect} (expected ${Object.keys(DIALECTS).join(' or ')})`);
  }

  const key = `${dialect} ${mode}`;
  let instance = instances.get(key);
  if (!instance) {
    const ajv = new DIALECTS[dialect]({ allErrors: true, strict: true, allowUnionTypes: true, ...MODES[mode] });
    addFormats(ajv);
    instance = { ajv, compiled: new WeakMap() };
...
//
// `true` and `false` are legal JSON Schema (accept everything / reject everything) but
// cannot key a WeakMap, so they compile fresh each call. Nothing in this repo uses a
// boolean schema; the branch exists so one does not surface as a phantom compile failure.
const validatorFor = (schema, instance) => {
  if (schema === null || typeof schema !== 'object') return instance.ajv.compile(schema);
 ...
 *
 * @param {object|boolean} schema - JSON Schema, in `options.dialect`
 * @param {*} data - value to validate; with mode 'coercing' it is mutated in place
 * @param {object} [options]
 * @param {'strict'|'coercing'} [options.mode] - see MODES above
 * @param {'draft-07'|'2020-12'} [options.dialect] - see DIALECTS above
 ...
 * @throws {RangeError} when `mode` or `dialect` names one that does not exist. That is a
 *   caller bug rather than a schema defect, so it must not be reported as `schemaError`
 *   and routed to a 500 that blames the schema.
 */
const validate = (schema, data, { mode = 'strict', dialect = 'draft-07' } = {}) => {
  const instance = instanceFor(mode, dialect);

  let validator;
  try {
    validator = validatorFor(schema, instance);
  } catch (err) {
    const message = `Schema failed to compile: ${err.message}`;
    return { valid: false, schemaError: true, message, errors: [{ path: '(schema)', message }] };
  }
  ...
  return { valid: false, errors, message: summarize(errors) };
};

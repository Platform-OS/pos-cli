 ...
 *
 * @param {object|boolean} schema - JSON Schema (draft-07)
 ...
 * @throws {RangeError} when `mode` names an instance that does not exist. That is a
...
const validate = (schema, data, { mode = 'strict' } = {}) => {
  ...
  if (!instance) {
    throw new RangeError(`Unknown validation mode: ${mode} (expected ${Object.keys(instances).join(' or ')})`);
  }

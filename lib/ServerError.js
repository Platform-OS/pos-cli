...
const showNotification = error => notifier.notify({ title: 'Error', message: error.message, icon });
...
const ServerError = {
  ...
  forbidden: error => {
    report(error, { extras: [{ key: 'type', value: 'ServerError' }] });
    showNotification(error);
    logger.Error(error.message, { hideTimestamp: true, exit: shouldExit(error) });
  },

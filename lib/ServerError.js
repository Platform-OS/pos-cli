...
const ServerError = {
  ...
  internal: error => {
    ...
    let message = JSON.stringify(error.error, null, 2);
    try {
      message = error.error.error.slice(0, 100);
    } catch(e) {
      logger.Debug('[ServerError: internal] error.error.error doesnt exist, using error.error');
    }
    ...
    report.error(error.error, { extras: [{ key: 'type', value: 'ServerError' }] });
    notifier.notify({ title: 'pos-cli Error', message, icon: path.resolve(__dirname, 'lib/pos-logo.png') });
  ...
  deploy: error => {
    ...
    const message = error.message ? error.message.slice(0, 100) : '';
    ...
    report.error(error, { extras: [{ key: 'type', value: 'ServerError' }] });
    notifier.notify({ title: 'Error', message, icon: path.resolve(__dirname, 'lib/pos-logo.png') });

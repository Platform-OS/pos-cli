  ...
  requestHandler: async (reason) => {
    ...
    switch (causeCode) {
      ...
      case 'ENETDOWN':
        ...
        await logger.Error(`${reason.cause}`, { exit: true });
        break;
      ...
      default:
        ...
        if (causeMessage && causeMessage.includes('getaddrinfo ENOTFOUND')) {
          ...
          if (hostnameMatch) {
            await logger.Error(`Could not resolve hostname: ${hostnameMatch[1]}`, { hideTimestamp: true });
          } else {
            await logger.Error('Request to the server failed.', { exit: false });
            await logger.Error(`${reason.cause}`, { exit: false });
          }
        } else {
          await logger.Error('Request to the server failed.', { exit: false });
          await logger.Error(`${reason.cause}`, { exit: false });

...
// request. `pos-cli-mcp` with no arguments still serves every tool. --no-http: these clients
// talk stdio, and an HTTP listener per session would only race the other sessions for its port.
const SERVERS = {
...
// The name each server used to be registered under. `platformos` said nothing about what the
// server was — the supervisor is platformOS too — so it is now `platformos-cli`, after the
// command it runs. An entry pos-cli wrote is renamed in place rather than added beside the old
// one: two entries for the same server would start it twice, and the agent would see every tool
...
const configureTool = async (toolId, rootPath) => {
  ...
  for (const [name, server] of Object.entries(SERVERS)) {
    ...
    // this loop iteration is about that entry, because adding the new name beside a customised
    // old one is the duplicate the rename exists to avoid.

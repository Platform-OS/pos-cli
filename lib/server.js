...
const start = (env, client) => {
  ...
  // INFO
  const info = (req, res) => {
    return res.send(JSON.stringify({ MPKIT_URL: env.MARKETPLACE_URL, version: package.version }));
  };
  ...
  legacy
    ...
    .on('error', err => {
      logger.Error(`Could not run the legacy admin panel at http://${host}:${parseInt(port)+1}`);
    });
};

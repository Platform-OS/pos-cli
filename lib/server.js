...
// `restartCommand` names the command the operator started, for the Gateway to quote when
// the instance asks for a two-factor session this run has not got. It matters here for the
// same reason it matters in watch mode: without it the Gateway steps up in place, and a
// step-up raises a readline prompt — from inside a running web server, where nobody is
// watching stdin and the browser request that triggered it hangs until someone types a
// code. With it the panel query fails with a message telling the operator to restart, and
// the terminal is left alone.
const start = (env, client, { restartCommand } = {}) => {
  ...
  app.get('/api/logs', (req, res) => {
    ...
    // Compatibility shim for GUI builds already installed in the wild. Versions of
    // gui/next up to this change send `?lastId=null` on the first poll (it built the
    // query with `args.last ?? null`), and Ajv will not coerce "null" to an integer.
    // The source has been fixed to send 0, so this can go once shipping a rebuilt
    // gui/next/build is acceptable — it exists only so an older GUI does not 400.
    if (['null', 'undefined', ''].includes(params.lastId)) delete params.lastId;

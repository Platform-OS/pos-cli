...
const start = (env, client) => {
  ...
  app.get('/api/logsv2', (req, res) => {
    gateway
      .logsv2({ key: req.query.key, from: 0, size: 10 })
      .then(body => { logger.Debug('test'); console.log(body); res.send(body) })
  ...
  next
    .listen(parseInt(port)+1, function() {
      logger.News(`---`);
      logger.News(`New admin panel preview: http://localhost:${parseInt(port)+1}`);
    })
    .on('error', err => {
      logger.Error(`Could not run the new admin panel preview at http://localhost:${parseInt(port)+1}`);
    });
};

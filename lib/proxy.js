...
class Gateway {
  ...
  logs(json, { signal } = {}) {
    ...
    return apiRequest({ uri: `${this.api_url}/logs?last_id=${lastId}`, json: true, forever: true, headers: this.defaultHeaders, signal });
  }
  ...
  sendManifest(manifest, releaseId) {
    ...
    return apiRequest({ method: 'POST', uri: `${this.api_url}/assets_manifest`, json, headers: this.defaultHeaders });
  }

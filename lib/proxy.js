...
class Gateway {
  constructor({ url, token, email, partner_portal_url, restartCommand }, client) {
    ...
    // Names only: this goes through the CLI logger, which does no redaction, and under DEBUG=1 an
    // MCP client collects the server's stderr — with the account email in the `From` header.
    logger.Debug(`Request headers: ${Object.keys(this.defaultHeaders).join(', ')}`);
  }
  ...
  async request(options) {
    ...
    try {
      ...
      return this.requestWaitingForPortal(withAuth);
    }
  }
  ...
  /**
   * `csv_export` selects which reader answers, and the two are exclusive: an export made with
   * `zip` answers 200 only with it, one made without `zip` only without it, and the wrong choice
   * is a 503 for an export that finished. Callers must know how the export was made.
   */
  dataExportStatus(id, csv_export = false) {
  ...
  /**
   * `csv_import` selects the reader that can answer for an import whose source was a ZIP; it does
   * not mean the data was CSV. A ZIP-sourced import answers 503 without it.
   */

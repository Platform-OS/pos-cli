...
class Gateway {
  constructor({ url, token, email, partner_portal_url, restartCommand }, client) {
    ...
    // Names only. This line goes through the CLI logger, which does no redaction, and with
    // DEBUG=1 the MCP server's stderr is collected by whichever client started it — so the
    // account email (the `From` header), and anything added here later, would travel with it.

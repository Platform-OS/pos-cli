...
// `asset` is the asset phase's own report; it arrives separately from the release
// report because assets are processed after the archive. It is shown as one more
// category so callers never have to know how the two are stitched together.
const printDeployReport = (deployReport, { verbose = false, asset } = {}) => {
  const fullReport = asset ? { ...deployReport, Asset: asset } : deployReport;
  if (!fullReport) return;

  // First pass: collect rows and find max widths for alignment
  const rows = [];
  for (const [category, data] of Object.entries(fullReport)) {

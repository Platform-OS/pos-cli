...
const start = async (env, directAssetsUpload) => {
  ...
  return gateway.ping().then(async () => {
    ...
    let subscription = await watcher.subscribe(process.cwd(), (error, events) => {
      console.log(events);
      console.log(error);
      ...
      normalizedEvents = normalizedEvents.filter((value, index, self) =>
        index === self.findIndex((t) => (
          t.path === value.path && t.type === value.type
        ))
      )
      ...
      syncTimeout = setTimeout(() => {
        for(let i = 0; i < normalizedEvents.length; i++){
          ...
          const isFile = path.extname(normalizedEvents[i].path);
  
          if(isFile){
            const relativePathToSync = path.relative(process.cwd(), normalizedEvents[i].path);
            if(normalizedEvents[i].type === 'delete'){

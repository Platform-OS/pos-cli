...
const Gateway = require('../lib/proxy'),
  ...
  watchFileExtensions = require('../lib/watch-files-extensions'),
...
const pushFile = (gateway, syncedFilePath) => {
  ...
  return gateway.sync(formData).then(body => {
    ...
    if (body) {
      const message = isAssetsPath(syncedFilePath) ? `[Sync] Synced asset: ${syncedFilePath}` : `[Sync] Synced: ${filePath}`;
...
const pushFileDirectAssets = async (gateway, syncedFilePath) => {
  if (isAssetsPath(syncedFilePath) && directUploadData) {
    try {
      await sendAsset(gateway, syncedFilePath);
    } catch (e) {
      // If direct upload fails, fall back to legacy sync
      logger.Debug(`[Sync] Direct upload failed, falling back to legacy sync`);
...
const start = async (env, directAssetsUpload, liveReload) => {
  ...
  return gateway.ping().then(async () => {
    ...
    if (directAssetsUpload) {
      try {
        await fetchDirectUploadData(gateway);
      } catch (e) {
        logger.Error('[Sync] Failed to fetch direct upload configuration. Falling back to standard sync.');

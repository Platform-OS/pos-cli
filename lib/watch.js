...
//         const collectedEventsUnique = [...new Map(collectedEvents.map(v => [JSON.stringify([v.path,v.type]), v])).values()]

//         for(let i = 0; i < collectedEventsUnique.length; i++){
//           const relativePathToSync = path.relative(process.cwd(), collectedEventsUnique[i].path);
//           if(collectedEventsUnique[i].type === 'delete'){

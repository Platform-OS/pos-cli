...
const shouldReload = filePath => {
  let topLevelDir = filePath.split(path.sep)[0];
  if (topLevelDir === dir.APP || topLevelDir === dir.MODULES) {
    const extension = path.extname(filePath);
    return watchFileExtensions.findIndex(currExtension => extension === '.' + currExtension) !== -1;
  }
...
const start = async (env, directAssetsUpload, liveReload) => {
  ...
  const reload = filePath => {
    if (liveReload && shouldReload(filePath)) {
  ...
  queue = Queue((task, callback) => {
    switch (task.op) {
      case "push":
        push(gateway, task.path).then(() => reload(task.path)).then(callback);
        break;
      case "delete":
        deleteFile(gateway, task.path).then(() => reload(task.path)).then(callback);
        break;
    }
  }, program.concurrency);

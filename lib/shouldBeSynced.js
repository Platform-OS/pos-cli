...
const isValidExtension = filePath => {
  ...
  if (!valid) {
    logger.Debug(`[Sync] Invalid extension of ${filePath}. Not syncing.`);
...
const isNotEmptyYML = filePath => {
  if (ext(filePath) === 'yml' && isEmpty(filePath)) {
    logger.Warn(`[Sync] Detected empty YML file: ${filePath}. Not syncing yet.`);
    return false;
  }
...
const isValidModuleFile = filePath => {
  ...
  if (filePath.startsWith('modules')) {
    return /^modules\/.*\/(public|private)/.test(filePath);
  }

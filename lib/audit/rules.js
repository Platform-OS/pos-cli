...
const baseRules = [
  {
    ...
    message: printMessage('please remove "enable_profiler: true" setting')
  },
  {
...
const deprecatedTagRules = DEPRECATED_TAGS.map(function(val) {
  return processRules(`\{\%\-*\s*${val}`, `deprecated "${val}" tag`);
});

const deprecatedFilterRules = DEPRECATED_FILTERS.map(function(val) {
  return processRules(`\\| ${val}`, `deprecated "${val}" filter`);
});

...
/*
  glob [string - glob]
    ...
    Omit `app` and `modules` in your definition
...
const detailed = [
  ...
  },
  {
    ...
    message: 'Please use snake_case naming convention in model names'
  }
];
...
module.exports = {
  audit: async () => {
    ...
    for (let rule of detailed) {
      const files = await glob(`{app,marketplace_builder,modules}/**/${rule.glob}`);

      for (let file of files) {
        const fileContents = fs.readFileSync(path.resolve(file), { encoding: 'utf8' });

...
/*
  ...
  test [string - regexp]
    What test will look for in the file contents. Make sure you escape as this is JS string (ie. doube backslash \)

  message [string]
    Message to be displayed if offence is found
*/
const detailed = [
  {
    name: 'profiler',
...
module.exports = {
  audit: async () => {
    ...
    for (let rule of detailed) {
      const files = await glob(`**/${rule.glob}`, {
        filesOnly: true
      });

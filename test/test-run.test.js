...
describe('Test run command', () => {
  describe('with mixed tests (passing and failing)', () => {
    ...
    test('exits with code 1 when running all tests and at least one fails', async () => {
      const { stdout, stderr, code } = await run('with-tests-module', 'staging');

      // Verify we ran multiple tests (the fixture has passing and failing tests)
      const output = stdout + stderr;
      expect(output).toMatch(/\d+ passed/);
      expect(output).toMatch(/\d+ failed/);

      // Exit code should be 1 when at least one test fails
      expect(code).toBe(1);
    });
  });

  describe('with only passing tests', () => {
    // Deploy the test fixtures with only passing tests
    beforeAll(async () => {
    ...
    test('exits with code 0 when running all tests and all pass', async () => {
      ...
      // Exit code should be 0 when all tests pass
      expect(code).toBe(0);
    });
  });
});

...
test('resolveDependencies ok', async () => {
  ...
  const modulesVersions = async (modulesNames) => {
    if(isDeepStrictEqual(modulesNames, ['payments_stripe', 'tests', 'a'])) {
      return [
    ...
    } else if(isDeepStrictEqual(modulesNames, ['payments', 'core', 'b'])){
      return [
        {'module':'payments','versions':{'1.0.0':{'dependencies':{'core':'1.6.0'}}}},
        {'module':'b','versions':{'1.0.0':{'dependencies':{'c':'1.0.0'}}}}
      ].concat(core);
    } else if(isDeepStrictEqual(modulesNames, ['core', 'c'])){
      return [
...
test('resolveDependencies do not use newest available version but the one defined in root', async () => {
  ...
  const modulesVersions = async (modulesNames) => {
    if(isDeepStrictEqual(modulesNames, ['tests', 'core'])) {
      return [tests, core];
    } else if(isDeepStrictEqual(modulesNames, ['tests'])) {
      return [tests];
    } else if(isDeepStrictEqual(modulesNames, ['core'])) {
      return [core];
    }
    throw new Error(`Unexpected modulesNames: ${JSON.stringify(modulesNames)}`);
  };
...
test('find module with newest version', async () => {
  const modulesVersions = async (_modulesNames) => {
...
test('find module with requested version', async () => {
  const modulesVersions = async (_modulesNames) => [{'module':'core','versions':{'1.0.0':{'dependencies':{}}, '1.5.0':{'dependencies':{}}}}];
...
test('find module with requested version even if it is beta', async () => {
  const modulesVersions = async (_modulesNames) => [{'module':'core','versions':{'1.0.0-beta.1':{'dependencies':{}}, '1.5.0':{'dependencies':{}}}}];

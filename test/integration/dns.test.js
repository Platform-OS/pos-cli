...
describe.skipIf(!hasSource)('pos-cli dns (integration)', () => {
  ...
  test('export produces a valid envelope from the live portal', async () => {
    const { code } = await cli([
      'dns', 'export',
      '--portal-url', SOURCE.portalUrl, '--token', SOURCE.token, '--instance-uuid', SOURCE.uuid,
      '-o', exportFile
    ]);
  ...
  test('offline dry-run import renders a plan without touching any portal', async () => {
    const { stdout, code } = await cli([
      'dns', 'import', '--file', exportFile, '--instance-uuid', 'fake-target-uuid', '--dry-run'
    ]);
    expect(code).toEqual(0);
  ...
  test('offline self-compare is clean', async () => {
    const { stdout, code } = await cli([
      'dns', 'compare', '--source-file', exportFile, '--target-file', exportFile
    ]);
    expect(code).toEqual(0);

...
* **An upload that needs more than five minutes no longer fails.** Node's HTTP client puts a 300-second ceiling on every request, counted over the whole *send* and never reset as bytes move, so a deploy, a `modules push`, a data import or an `uploads push` whose archive took longer than that was cut off — and reported as `fetch failed`, three words naming neither the host nor the cause. Measured: a 96MB body going out at a steady 256KB/s, with a healthy server at the other end, was killed at 300.9s after 64MB had arrived. In practice a 50MB asset archive needed more than 1.4 Mbit/s of upload sustained, and 500MB needed 13 Mbit/s, so the deploys this hit were exactly the ones on a slow or shared link. Requests that send a file now run under a ceiling of their own and are given twenty minutes; everything else is unchanged, because for a small JSON call five minutes of silence really is a failure. A transfer that stalls is still ended rather than left hanging, and a timeout of either kind is now reported as a timeout, with the host it was for.

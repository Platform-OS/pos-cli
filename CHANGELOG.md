...
* **A request that is never answered now ends by itself.** Nothing passed a timeout to `fetch`, so an instance that accepted a connection and then said nothing held the call for as long as the process lived. On the command line a person presses Ctrl-C; the MCP server is long-lived, answers concurrently, and its cancellation signal fires only when the *client* gives up — which an agent waiting on a tool result does not do. `page-fetch` was the sharpest case, since a GET runs that page's Liquid.
  ...
  Every request made through `apiRequest` now has a deadline, decided in one place. It bounds **time to first byte**, not the transfer: the timer is cleared the moment response headers arrive, so a slow body is never cut off — `AbortSignal.timeout` would have been a whole-response deadline, which aborts exactly the transfers that are working. A request whose body is a file gets a longer bound, because its headers cannot arrive until the upload has, and a caller that knows better passes `timeoutMs`.

...
* **A request that is never answered now ends by itself, and says so.** Nothing passed a timeout to `fetch`, so an instance that accepted a connection and then said nothing held the call for as long as the process lived. On the command line a person presses Ctrl-C; the MCP server is long-lived, answers concurrently, and its cancellation signal fires only when the *client* gives up — which an agent waiting on a tool result does not do. `page-fetch` was the sharpest case, since a GET runs that page's Liquid.
  ...
  **The bound is 4.5 minutes because Node's HTTP client caps every request at five**, counted over the whole send and never reset as bytes move — measured, a healthy 96MB upload at a steady 256KB/s was cut at 300.9s with 64MB already delivered, and nothing a request carries can raise that. What reached the operator when it happened was `Request to the server failed.` — no host, no duration, nothing to say it was a timeout, and indistinguishable from a DNS failure. Staying under the cap is what makes the failure pos-cli's own to report: it now arrives as a timeout naming the host and the wait, and over MCP as `kind: unavailable` for that host rather than as an unexplained transport error. Cancelling by the caller is still reported as itself rather than as a timeout.

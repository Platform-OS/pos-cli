...
* Tools that only read — `envs-list`, `logs-fetch`, `job-status`, the deprecated deploy/data/test status tools, `data-validate`, `constants-list`, `generators-list`, `generators-help`, `migrations-list` and the Partner Portal lookups — carry `annotations.readOnlyHint`, which clients may use to run them without asking. Everything else is unmarked, which means "may change things".
...
* **The `endpoint` argument is gone from `deploy-status`, `deploy-wait` and `logs-fetch`.** It replaced the URL the request went to while the `.pos` token was still sent, so a caller that could reach the server — including a model persuaded by the content of a page, a log line or a data file it had just read — could name any host and have this machine's platformOS credentials delivered to it. The request URL now always comes from the resolved credentials. The three schemas are closed, so a call that still passes `endpoint` is rejected with `INVALID_PARAMS` rather than quietly ignoring it. Other tools still accept `endpoint`; that is being removed separately.
...
* The documentation no longer claims the HTTP transport requires a Bearer token, API key or rate limit; none of those were ever implemented. `docs/SSE_GUIDE.md`, `docs/API.md` and `mcp-min/README.md` now describe the actual protection and its limits.

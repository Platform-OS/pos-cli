...
* **`logs-fetch` said `{% log %}` output was not in the stream. It is, when deployed code writes it.** That claim was generalised from a measurement taken only through `liquid-exec`, and an agent evaluation opened on `{% log %}` rows from a deployed page. Re-measured: a bare `{% log %}`, a `type:`-tagged one and a failing filter, all rendered through `liquid-exec`, produce no rows at all — so what decides it is where the code ran, not how it logged. The description now says the useful half: deployed code reaches the error log and a `liquid-exec` render never does, not even its own errors, so debugging a template that way leaves nothing to go looking for. Both corrections together *reduced* `tools/list` by 126 bytes.
...
* **`logs-fetch` returned a `lastId` its own schema refused, so the resume it documents had no correct input.** A row id is a microsecond epoch — `"1790008926.7639065"`, a string in every row the instance sends — and the tool returned `Number(...)` of it while declaring the parameter an `integer`: three types for one identifier. Handing the value straight back, which is exactly what the description said to do, failed with *"/lastId must be integer"*, and the only value the schema did accept, `1790008926`, re-delivered every row from that second, because `last_id` is a strict greater-than. Tailing logs — the reason to page them — could not be done correctly at all. The cursor is now the string the instance sent, at every step, and the schema accepts it. The description also says what it never did: which stream this reads (the error log that `pos-cli logs` tails, which does not carry `{% log %}` output), and that `limit` counts from the **oldest** row after the cursor.

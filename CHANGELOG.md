...
* **`deploy-dry-run`: what a deploy would change, before it changes it.** A deploy that is not partial is the whole intended state of the instance — every file missing from the build is deleted there — and over MCP the only way to find that out was to do it. The new tool builds the release, uploads it with `dry_run`, and reports which files would be added, updated and deleted, applying nothing; assets are covered too, since the manifest is validated against the dry-run release without anything reaching S3. `deleted`, `upserted` and `skipped` are totals an agent can branch on, with the per-category breakdown beside them. It is a separate tool rather than a `dryRun` flag on `deploy-start` so that no argument can turn it into a deploy, and so the two can carry different annotations: `deploy-start` stays `destructiveHint: true`, which a client may gate behind a prompt, while the dry run states `destructiveHint: false` — and deliberately not `readOnlyHint`, because it does record a release and write an archive. It joins `--profile dev`: without it, an agent in that profile has `deploy-start` and no way to see the blast radius first.
  ...
  **A preview leaves a release behind, and that is the platform's intent rather than a leak.** `dry_run=true` allocates an id, validates against it and keeps the record — `status: success`, `deleted_at: null`, one id per preview, so previews outnumber deploys wherever the dry run is run before every one. Measured on 2026-09-23 against a real deploy on the same instance, the two are distinguishable: a preview is stamped `options.dry_run: "true"` where a deploy's is `null`. `status` alone cannot tell them apart, since both say `success`. pos-cli does not delete or hide these records, and no GraphQL query exposes them at all.
...
* **`page-fetch`: ask the instance for the page a visitor gets.** The last step of the loop this server is built around — edit, check, deploy, **verify** — was the one an agent could not take without leaving the server. Reading a file back proves it reached the instance; routing, the layout, the authorization policies and every partial the page renders sit between that and a URL that works. `page-fetch` takes a path and reports the status, the headers worth reading and the body, as an anonymous visitor gets them. A `404` is a successful call reporting a page that is not live, not a failed one. A `3xx` is reported with `isRedirect: true` and `headers.location`, never followed — named `isRedirect` because on a Fetch `Response`, `redirected` means the opposite, that the redirect *was* followed. A response that is not text is described rather than returned, and when it declares a `content-length` its body is released unread: the size is the whole answer, so an asset an agent pointed this at is not pulled into the server to be counted and thrown away. It is in `--profile dev`.

  The host is the resolved credentials' host and nothing a caller passes can move it: the schema requires a single leading `/`, and the URL built from the path must still have the instance's origin — `//elsewhere.example.com` reads as a path and is another host to `new URL`, so the origin comparison is the check that actually holds the rule. No credentials are sent, so a redirect, which is reported and never followed, cannot carry one anywhere. The body is capped at 16 KB and cut on a character boundary; a response that is not text is described rather than returned.

* **`check-run` says whether `autoFix` had anything to do — and the honest answer today is no.** `autoFixed: false` alone could mean "nothing here is auto-fixable" or "the pass did not run", and an agent had to diff its own working tree to tell them apart, on the one tool here that writes to disk. The result now carries `fixable` — how many of the reported offenses could be corrected automatically — computed **whether or not `autoFix` was asked for**, which it previously was not: gated on the flag, a plain lint reported that nothing was fixable when it had simply never looked. `fixedFiles` names what was written, and is absent when nothing was. Investigating it turned up the real answer to the evaluation's question: **not one of platformos-check's 54 checks builds a correction**, so `autoFix` cannot write anything whatever it is pointed at. The plumbing stays — the fix is upstream's to ship — and a test fails if it ever does.

* **`deploy-dry-run` says that some categories are counted and not named.** The API answers a category with either the paths or a bare count, so `count` can exceed `files` — an evaluation read that as a bug and spent a separate `admin_assets` query establishing that its asset really had gone out.

* **The server instructions say where deployable source has to live.** `app/` — `views/pages`, `views/partials`, `views/layouts`, `assets`, `schema`, `graphql`, `lib` — and that a path matching none of it is not deployed. This was the one place an agent evaluation said it came close to needing pos-cli's own source: it guessed the layout from prior platformOS knowledge and confirmed each guess with a `deploy-dry-run`, which is a round trip to the instance and a release id per guess. It belongs to the project rather than to any one tool, so it goes in the instructions, charged once a session instead of in every tool description on every request.

* **`tests-run-async` is removed. It never worked, and it started the run anyway.** The tool asked `/_tests/run_async` for a run id; the tests module answers with a `test_name` and no id, so every call failed with `MISSING_ID` — *after* the instance had begun executing the suite, with the handle dropped. `job-status` then polled `/_tests/results/:id`, an endpoint that has never existed in that module's history. An agent evaluation found this by reading the logs for an unrelated reason and seeing its own test run in them. `unit-tests-run` now runs the whole suite when `name` is omitted, which is the capability this was reached for; the `test-run` job kind, its adapter and `mcp-min/tests/result.js` go with it, and `tools.config.json` keeps a tombstone so a config naming it warns rather than fails. Rebuilding it on the module's own contract — the summary arrives in the log, typed `<test_name> SUMMARY`, which is parseable to at least 103 KB — is recorded in `docs/MCP_COVERAGE.md` as a separate decision rather than done here.

* **A dry run spent 88% of its answer listing files that were not changing, twice over.** `deploy-dry-run` published `deleted`/`upserted`/`skipped` as flat lists *and* repeated every one of those paths under `byCategory` — measured against a live instance, the two sets were identical, and on a project of 82 files where **nothing changed** they were 6,820 of a 7,770-byte answer. The names are now kept once. `byCategory` carries per-category counts, `skipped` carries a count and no names — a path that is not changing is the one thing nobody asked this tool about — and `upserted`, `deleted` and `discarded` keep theirs, which is what the tool exists to report. **The same no-op dry run is now 575 bytes**, and a non-partial run still names exactly the files it would delete.

* **`job-status` repeated the whole file report a third time.** 3,660 of a completed deploy's 4,438 bytes were `result.release.report`, listing files that did not change *after* the deploy, when nothing can be done about them. `report` and `asset_report` are now counts. **The rest of the release record is still forwarded verbatim** — all sixteen fields, including ones nothing here knows about — because an allowlist has to be maintained and a field the platform adds goes silently missing, which is how a discarded file stayed invisible for a release. This is one named field with a measured cost, not a list of what is allowed through. A completed deploy is now **1,023 bytes**.

* **`graphql-exec` says the schema is introspectable.** An agent evaluation reached `admin_liquid_partials` unprompted and then lost its first call to `Field 'name' doesn't exist on type 'LiquidPartial'` — the field is `path` — recovering only because it thought to try `__type`. Naming more `admin_*` families would have bought nothing, since the wildcard already worked; 90 bytes saying the field names can be discovered is what was missing.

* **A failing test was reported as a retryable server error, and a test name that matched nothing was reported as a pass.** `unit-tests-run` asked `/_tests/run?formatter=text` and parsed the prose — but that endpoint has never answered prose: it serves the `.js` page and ignores `formatter`, so ~180 lines of text parser ran against JSON and invented a test out of every non-indented line. That is how a run matching nothing answered `passed: true, totalTests: 1` while the payload underneath said `total_tests: 0`. The tool now requests `/_tests/run.js` and reads the JSON, and the parser is gone.

  Three behaviours change with it. **A failing assertion is a completed run**: the runner answers HTTP 500 for a red build — deliberate, and how it signals CI — so the body is read before the status is judged, and the result is `ok: true` with `passed: false` instead of `kind: unavailable`, which the server instructions define as "the same call may work later". **Nothing matched is not a pass**: a `name` that matches no test is `NO_TESTS_MATCHED`, no name with no tests at all is `NO_TESTS`, and both name the GraphQL query that lists the test files, since no tool does. **The failures come back**: `tests[]` carries each test's name, assertion count and the assertions that failed, bounded at 2,000 characters per message — one test comparing two large objects produced a 103 KB result before that cap and 4.8 KB after.

  `name` is no longer required — omit it to run every test, which is what the runner does with no filter. It used to be mandatory, with the description sending whole-suite runs to `tests-run-async`, a tool that answers `MISSING_ID` against tests@1.3.5 while running the suite anyway. `path` is removed rather than accepted and ignored: the tests module filters on `name` alone and never read it, so a run narrowed with `path` quietly ran everything. A module older than tests 1.1.0 serves no `/_tests/run.js` and now answers `TESTS_MODULE_OUTDATED` with the command that updates it, instead of an unexplained 404.

* **`page-fetch` no longer asks for credentials it does not send.** Its description says no credentials are sent, and it uses nothing but the host — yet `resolveAuth` refused `url` without `email` and `token`, so pointing it at an instance you have a URL for and no account on was impossible. The triple is never checked against anything, so the requirement stopped nobody and blocked the honest caller. `url` now stands alone on this tool, and `email`/`token` are gone from its schema rather than accepted and ignored: the shared `url` description reads "with email and token", which here would be false. Every other resolution step, and the guard that makes a call name its instance, are unchanged.

* **`logs-fetch` can find one error without reading every row.** It took `lastId` and `limit` and nothing else, so an agent asked to find a specific error had to page forward from the beginning and pay for every row on the way. Three parameters change that: `since` (an ISO-8601 time), `errorType` and `contains`, the last two matched as case-insensitive substrings — `errorType: "error"` finds a row typed `Liquid error`, which `pos-cli logs --filter error` does not.

  **`since` costs nothing; the other two cost a read.** The instance cannot narrow anything: measured on 2026-09-22, `/logs` answers `error_type`, `q`, `search`, `order` and `limit` byte-identically to a parameter it has never heard of, so `errorType` and `contains` are matched in the tool after the rows arrive. What that buys is context, not bandwidth — ten thousand rows fetched, twelve returned — and the tool description says so, because a narrow search over a large log takes as long as reading it. `since` is different: a row id is a microsecond epoch of the instant `created_at` names, and `last_id` is a strict greater-than, so a timestamp becomes a cursor the instance itself skips by and nothing is fetched to be discarded.

  A call now reads at most 10,000 rows, the same ceiling `limit` already had; a read that stopped there says `scanLimitReached`, so "nothing matched" and "stopped looking" are different answers, and `lastId` carries on from where it stopped. The cursor advances over every row read rather than only the ones returned, so a filtered read resumes *after* the rows it rejected. `since` and `lastId` together are refused rather than resolved by precedence — a caller that passed both meant one of them. This was the answer to the gap TASK-28 was opened for; the `logsv2 search` it planned is still the better tool and is still unreachable (TASK-56).

* **`pos-cli fetch-logs`: dead code removed, `-q` fixed, and the paging cursor no longer rounds.** Four defects. `--last-log-id` was read through three nested ternaries that all evaluated to the same thing and then through `options['last-log-id']`, a key commander never sets because it camel-cases option names — it worked by accident, and is now one assignment. `-q` suppressed nothing: the catch read `program.quiet` off the imported commander program instead of the parsed options, so it was always `undefined`.

  **The cursor was advanced with `Number(row.id) > Number(maxId)`.** A row id is a microsecond epoch with up to 17 significant digits, which is the edge of what a double holds, so two adjacent ids can round to one value — the cursor then stops moving, the loop's no-progress check fires, and the fetch ends early having silently skipped the rest of the log. Ordering now goes through `lib/logRowId.js`, which compares the digits and never parses an id: whole parts by width then by value, fractions padded and compared by position. `logs-fetch` used the same float comparison and now shares the helper. The command also says when it stopped early — an instance answering with entries but none newer than the cursor cannot be paged further, and that is not the same as reaching the end of the log.

  **`--endpoint` is kept and now says what it does.** It sends your stored instance token to whatever host it names. That is the parameter class removed from every MCP tool, but on a command line the operator chooses the host, so it stays — with its purpose stated in `--help` and the README, and a line on stderr naming where the token is going. `-q` silences that too. The README's example rows were also wrong: they showed integer ids and a `type` field, where a real row carries a decimal `id` and `error_type`, which is how a reader ended up building the integer cursor this entry is about.

* **`logs-fetch` describes what actually reaches the error log.** It has been stated wrongly twice, in opposite directions: first that the stream carries no `{% log %}` output at all, generalised from a single context; then that a `liquid-exec` render never appears in it, not even its errors. Measured on a healthy instance: deployed code writes there, `{% log %}` included, and a `liquid-exec` render's **Liquid errors** are written too — only its `{% log %}` is not. So a template debugged through `liquid-exec` reports its failures in the log and leaves no trace of its own logging, which is the half worth knowing.

* **`graphql-exec` now says that it can read an instance back.** The instance has always served an admin read API over GraphQL — `admin_pages` with each page's `content`, `admin_liquid_partials` with each `body`, `admin_assets`, and the schemas, forms and policies beside them — and `graphql-exec` has always been able to reach all of it. Nothing said so, so an evaluation of this server concluded the capability did not exist, deleted a page it had never seen, and could not enumerate what else was on the instance. That is how an agent sees what a non-partial deploy would delete, before it deletes it. Naming the family costs 128 bytes once; a tool wrapping it would have cost 450–900 on every request to duplicate what was already there.

* **`url`, `email` and `token` are described.** They shipped with no `description` at all, beside a documented `env`, on every tool that authenticates — including `deploy-start`. From the server alone an agent could not tell whether they replace `env` or override it, whether all three are needed, or what wins when both are passed, so an evaluation of this server declined to use them: the one argument it would not guess at is the one that decides which instance a deploy reaches. `url` now says the three go together and are used instead of `env`; `email` and `token` name their partners. It is not free — `authProperties` is spread into twenty-one tools and each publishes its own copy, so three short sentences are 3,420 bytes of a bare `tools/list` and 1,524 of `--profile dev`. Saying it once in the server instructions would have cost about a fifteenth of that, and would have left the highest-stakes parameters undescribed at the moment an agent is deciding what to put in them.

* **`check-run`'s `fileCount` is now `filesWithOffenses`.** It sat beside `filesChecked` with nothing saying which was which, so a clean run answered `fileCount: 0, filesChecked: 2` — which reads as "nothing was checked". An evaluation decoded it only by breaking its own code and watching the number move.

* **`logs-fetch` rows carry what the instance said and no more.** `data` is omitted when it is null and `updated_at` when it repeats `created_at`: 52 bytes of a measured 322-byte row, on a tool whose `limit` goes to 10,000. Both are dropped per row and only when empty, so a row that does carry `data`, or that was updated after it was written, keeps them — this cannot lose something the instance meant.

* `job-status` documents the two words it answers with: `state` is this server's (`running`, `completed`, `failed`, the same three whatever was started) and `status` is the instance's own word for the same moment, which is also what `deploy-start` returns. `result.release` stays the release record verbatim — 815 bytes against 566 for the fields anything reads — and that 249 is recorded as a deliberate cost, because an allowlist would have to be maintained and a field the platform adds would go silently missing, which this tool has already had happen once. Two of its fields are noted rather than read: `name` is always `"User import"` whatever the release, and `module_deployment` tracked `options.partial_deployment` exactly across eight sampled releases.
...
* **The pre-SDK HTTP API is removed**: `GET /`, `GET /tools`, `POST /call` and `POST /call-stream`. `POST /mcp` — MCP Streamable HTTP, serving 2026-07-28 and the 2025 revisions — replaces all of it, and `GET /health` is unchanged. Nothing that speaks MCP used those routes: MCP clients connect over stdio or to `/mcp`, and `pos-cli ai init` writes `--no-http`, so a standard install never opened the listener at all. What they served was a bespoke JSON API on a loopback port with no authentication, whose documentation and example clients this release also deletes. `/call-stream`'s tool path could no longer do anything in any case, since no registered tool implements `streamHandler`. Going with them: `mcp-min/sse.js`, the SSE session registry, and the JSON body parser — `/mcp` reads its own body — which leaves `http-server.js` as the bind, the Host/Origin check and two routes. The loopback default, `Host`/`Origin` validation and bind-failure reporting are unchanged and still apply to everything the server answers.
...
* **A request that is never answered now ends by itself.** Nothing passed a timeout to `fetch`, so an instance that accepted a connection and then said nothing held the call for as long as the process lived. On the command line a person presses Ctrl-C; the MCP server is long-lived, answers concurrently, and its cancellation signal fires only when the *client* gives up — which an agent waiting on a tool result does not do. `page-fetch` was the sharpest case, since a GET runs that page's Liquid.

  Every request made through `apiRequest` now has a deadline, decided in one place. It bounds **time to first byte**, not the transfer: the timer is cleared the moment response headers arrive, so a slow body is never cut off — `AbortSignal.timeout` would have been a whole-response deadline, which aborts exactly the transfers that are working. A request that sends a file — a release archive, or an asset archive on its way to object storage as raw bytes or as a presigned form post — asks for a longer bound, because its headers cannot arrive until the upload has. It does not get it yet: Node's own HTTP client caps the whole send at five minutes and does not reset that as bytes move, so an upload needing longer than five minutes fails today regardless (measured: a healthy 96MB upload at 256KB/s was cut at 300.9s). Raising that ceiling is a separate change. A caller that knows its endpoint passes `timeout`: the Partner Portal and the presign service answer JSON in milliseconds, so they wait 30 seconds rather than five minutes.

  It is a backstop rather than a latency target: 5 minutes for an ordinary call, 15 for one carrying a file, against a measured 2.4s for a full 39-test suite. A timed-out request arrives as `kind: unavailable` with the host named, the same as a refused connection, and cancelling by the caller is still reported as itself rather than as a timeout. `page-fetch` shares the same bound instead of carrying its own.

* **An instance with no tests is told how to find out what goes in one.** Two evaluations could not write a test file from anything the server said, and both recovered the same way — by reading the `tests` module's assertions back off the instance. Those are partials carrying a `{% doc %}` block that names every parameter, so `NO_TESTS` now points at them rather than restating a contract that belongs to that module and would be wrong here the first time it changed. `NO_TESTS_MATCHED` deliberately does not: where tests already exist, real ones are the better example and the query it carries names them.

  **No MCP resources capability was added, and the decision is recorded** (`docs/MCP_COVERAGE.md`). The gap turned out to be one item rather than a corpus: measured 2026-09-23, a page with no front matter at all deploys and serves at a slug taken from its filename, and a wrong table schema is refused by the converter with the valid properties named — so two of the three things resources were being considered for are not gaps.

* **A dry run names the deletions that cost records.** A deploy that is not partial deletes every file missing from the build, and in the flat `deleted` list a table schema read exactly like a partial — but dropping `app/schema/*.yml` drops the rows in that table, which are not in the archive and which no second deploy puts back. `deploy-dry-run` now answers `dataLoss` beside `deleted`, naming that subset, and only when there is one. It is keyed on the converter's own category (`Tables`, measured 2026-09-23), so a category pos-cli does not recognise is never flagged — silence means "not known to destroy data", not "safe".

  Measured while doing it, and now written down: **a non-partial deploy does not delete assets.** An asset on the instance and absent from the project does not appear in `deleted` at all, and no `Asset` category appears for deletions. An evaluation had inferred this from `Asset deleted: 0` and had no way to confirm it.

* **`logs-fetch` says which kind of empty an answer was.** `count: 0` reads the same whether nothing happened in the window asked about or the instance stopped writing to its log days ago — an evaluation hit the second and reported the tool as broken, reasonably, because nothing in the result distinguishes them. A `since` read that finds nothing now also reports `newestRow`: the newest row the instance holds, or `null` when it holds none. A timestamp far in the past says the log is stale rather than quiet. It costs one extra read and only on that path: a `lastId` tail sits at the tip of the stream and is empty most times it is called, so it is never charged there, and an instance that will not answer leaves the field off rather than failing the call.

* **Six defects an agent evaluation found in the tools themselves, and the guard for the class each belongs to.** Round 3 exercised `--profile dev` end to end; these are what it hit that were ours.

  **A test that *raises* was reported as "unavailable, retry later".** A test is a Liquid partial, so one that raises takes the page down with it and the runner answers 500 with an HTML error page — the same shape an unwell instance gives, and read from the status alone that is "the same call may work later". An agent was told to retry a test that could never pass, with no route to the reason. The instance is asked a second question instead, the way a 5xx on a job already is: one that answers `getInstance` while failing to run a test is not the thing that failed, so this is now `kind: project`, `TEST_RUN_CRASHED`, naming the filter the run was given. The probe is bounded at 5 s and only ever runs after a 5xx, so a run that works pays nothing and a diagnostic cannot be what makes a failure slow.

  **`liquid-exec` blamed the instance for the caller's template.** Every failure answered `kind: instance` — "the instance ran it and refused; read the message rather than retrying unchanged" — including a template that never parsed, which is the one failure a caller can always fix and resend. Measured against a live instance on 2026-09-23, `diagnostic.type` settles exactly two cases: `Liquid::SyntaxError` (nothing ran) and `Liquid::UndefinedFilter` (a filter name that does not exist) are now `kind: input`, `LIQUID_TEMPLATE_ERROR`. It settles no more than that, and the rest deliberately keep `instance`: `GraphqlTagError` is returned both for a `.graphql` file that is not there and for a query the instance refused on its own data rules, and `Liquid::Error` is the base class. Guessing either way would tell an agent to rewrite a template that is already correct.

  **`job-status` advertised an operation that no longer exists.** Its description listed "a deploy, a data import, export or clean, or an async test run": three of those `--profile dev` hides, and `tests-run-async` was removed in this release while the description outlived it — there is no test job kind in `JOB_KINDS` at all. It now names no kinds, which is also why it cannot drift again: a caller has the `job_id`, and the kinds were never its to know. The check that was meant to catch this only matched hyphenated tool names, so prose slipped past it; it now also refuses the spaced form of a hidden tool's name.

  **Every tool result was pretty-printed.** `JSON.stringify(value, null, 2)` on every result of every call, from a server that counts its tool descriptions to the byte — 27% of a `logs-fetch` of twenty rows was indentation. Results are compact now; nothing but a model reads them.

  **`page-fetch` published credentials it does not send.** Its description says no credentials are sent, and it uses nothing but the host — yet resolving a named environment put the email and a masked token in `meta.auth`, flatly contradicting it. An anonymous resolution now records only the host, in `resolveAuth` rather than at the call site, so a second anonymous tool cannot forget to.

  **A remedy that misled.** `NO_TESTS_MATCHED` suggests a query listing the test files; it had no `per_page`, so on an instance with 38 tests it returned the default 20 with nothing saying so, and an agent following it concluded its test did not exist. It asks for `per_page: 100` and `total_entries`.

* **`pos-cli logs --filter` stopped reading forward, and silently dropped the rows it was asked for.** Only a row that survived the filter advanced the cursor, so between two matching rows the stream did not move at all. Driving the real command against a stand-in instance serving five fresh non-matching rows per poll, six polls asked `last_id=0` every time, where an unfiltered run walked forward normally. `last_id=0` returns the newest 20 rows as a sliding window — measured 2026-09-23 — so a filtered run was pinned to that window and never examined anything that scrolled out of it between polls. On a busy instance `--filter error` therefore lost errors, which is the one thing it is used for. The cursor now advances over every row read and the filter decides only what is printed, which is what `logs-fetch` already does.

  **Two more in the same file.** The dedup kept every log row object for the life of the process although only the ids were ever read — a tail left running all day grew without bound; it is a `Set` of ids now, and the cursor goes through `lib/logRowId.js` rather than taking whichever row came last. And the desktop notification had been dead since July 2019: a refactor began destructuring the row, and `isError` was left reading `error_type` off the **message string**, so it was always `undefined` and no row ever took the error branch. That also meant error rows lost their colour and, once a structured Liquid diagnostic was added, printed without it — the diagnostic lived in the branch an error could not reach.

  **Error rows are red again and now write to stderr**, where `logger.Error` has always sent them, with their diagnostic following them onto the same stream rather than being split across two. They do **not** raise a desktop notification: `logger.Error` sends one by default, and one per error row on a tail that prints every row is a stream of popups for what is already on screen. Scripts reading `pos-cli logs` on stdout will no longer see error rows there; `pos-cli fetch-logs` remains the command built for scripting.

* **Both deploy tools now say which project they deployed.** `deploy-start` and `deploy-dry-run` resolve `app/`, `marketplace_builder/` and `modules/` against the server's own working directory, and no argument can name a different one — an MCP server is started by an editor, so its directory is whatever launched it. Neither tool reported that directory, so "which project am I deploying?" was answerable only by calling `check-run`, which echoes the path it linted; an evaluation confirmed its directory exactly that way, by accident of a different tool. Both now return `appPath`.

  It matters because a deploy that is not partial deletes every file missing from the build, and `partial` defaults to `false` — so the destructive mode is reached by omitting an argument. Measured on a live instance: from a project holding one page, a default `deploy-dry-run` against an instance holding two others reports it would delete both and answers `verdict: would_succeed`. Nothing is wrong from the instance's point of view, and the existing `EMPTY_ARCHIVE` guard does not fire, because the project is not empty — it only catches a directory with nothing deployable in it at all.

  **The default stays `false`**, and the reasoning is now recorded where it is set: it is what `pos-cli deploy` does, so a deploy means the same thing on both surfaces, and defaulting to `partial` would leave stale files behind silently — a different wrong answer. What stands in for a deliberate choice is `deploy-dry-run`, which names the delete list before anything is applied, and now the project that list belongs to.

* **A GraphQL document the instance never ran was reported as the instance refusing it.** `graphql-exec` answered every failure `kind: instance`, which the server instructions define as "the instance ran it and refused; the message says why" — so a client routing on `kind`, which is the whole point of `kind`, was told to read the message and not to rewrite a query that had a syntax error in it. An evaluation hit this on its first call, twice.

  A malformed document, an unknown field or argument, a variable given an invalid value, and several operations with none named are now `kind: input`, `code: GRAPHQL_DOCUMENT_ERROR` — a failure the caller corrects and sends again. A document that ran and was refused on the instance's own data rules stays `kind: instance`, `code: GRAPHQL_EXEC_ERROR`. Both keep the instance's own message, `locations` and `path` exactly as before; that part was already right.

  **The two are told apart by the response, not by the message.** GraphQL omits `data` from a response whose request failed before execution and sends it, `null` included, once execution has begun, so the presence of the key answers the question outright. Measured across nineteen cases against a live instance on 2026-09-23: all eight parse and validation failures omitted `data`, all three runtime failures sent it, and a rejected token never gets here at all because it is an HTTP 401. Reading `extensions.code` — the shape this was expected to need — would have misclassified a parse error and an invalid variable, which carry no `extensions` at all.

* **The test runner's log stream decided what it had already printed by ordering the row id.** `lib/test-runner/logStream.js` seeded its cursor as the number `0`, replaced it with a string after the first row, and then used `row.id <= this.lastId` for both questions it had to answer: whether a row had been handled, and what cursor to send next. A row id is a microsecond epoch with a fraction whose length varies, so after the first row that comparison is lexicographic — right only while two fractions are the same length. Where one is a prefix of the other (`.76` against `.7600000`, the same instant) it reads an already-handled row as new, or hides one that is new.

  It is `logs-fetch`'s arrangement now: a `Set` of ids already handled decides what to print, and `newerOf` (`lib/logRowId.js`) decides the cursor, so neither question is answered by sorting a string. That also fixes the cursor moving *backwards* on a page returned out of order, which the old code and a plain "last row wins" both do — on a stream that only ever polls forward, a cursor behind rows it has already printed never recovers, and the run reports a timeout rather than the result it was waiting for. `lib/test-runner/logStream.js` was the last of the four log readers still ordering an id itself.

* **`liquid-exec` sent a whole rendered page to the model, twice, when a render failed.** A template can fail *after* building most of a page: the endpoint then answers with its own `error` null and `Liquid error (line N): …` rendered into the output. The tool put the entire endpoint response in `details`, and built the message from the rendered output when there was no `error` field — so the page arrived as `details.result` and again as `error.message`, unbounded, while the model was already handling a failure. The message is now the error lines lifted out of that output: distinct lines only, because one failing partial in a loop renders the same line once per iteration, at most ten of them, and each ends at the next newline, the next tag or 200 characters — rendered HTML is frequently one long line, so stopping only at a newline took the rest of the document with it.

  **Every top-level string in an error's `details` is now capped at 4,096 characters, not just `body`.** Naming the fields to bound is an allowlist the next thrower will not know about, which is the same reason the bound lives in `toResult` rather than at each thrower. `body` keeps the rule it already had — an HTML error page is reduced to its `<title>`, and parsed JSON is never cut — and nested values are left alone, so a structured upstream record still arrives whole. `MAX_ERROR_BODY_LENGTH` is `MAX_DETAILS_STRING_LENGTH`, since it no longer bounds only bodies.

* **A file the deploy threw away was reported as an unqualified success.** The platformOS converter discards a file whose path matches no part of the layout — anything under `app/tests`, for one — and the release still answers `status: "success"`. The instance *does* say which files: in `warning.files_not_matched`, four levels down inside the release record, while `ok`, `state`, `status` and `done` all said the deploy had landed. An agent making any reasonable check concluded the file was on the instance. `job-status` now names the discarded paths in `data.warnings`, beside `state` rather than under `result`, and the tool's description says to read `warnings` even on a completed job. `state` is still `completed`, because it is true — the deploy finished, and what the converter kept is a fact about the project rather than a failed operation — so the warning is what carries it. The readable list is capped at ten paths; `result.release.warning` still carries every one, and a warning key this release does not recognise is passed through rather than dropped — a `warning` that arrives as something other than an object is ignored rather than walked, since `Object.entries` over a string yields one entry per character.

* **`deploy-dry-run` reports the same files as `discarded`, before the deploy instead of after it** — which is what a dry run is for. It is always present, so `discarded.count === 0` means nothing would be dropped rather than "this tool does not answer that", and it does not move `verdict`: the deploy really would succeed, minus those files.

* **`unit-tests-run` pointed its example at a directory the deploy discards.** Its `path` example is `tests/users`, which is a path inside the test runner's namespace — but read as a file path it sends you to `app/tests/`, which is exactly where an evaluation put a test that was then silently dropped. The parameter now says test files live under `app/lib` and that a deploy discards `app/tests` without failing.

* **`logs-fetch` returned a `lastId` its own schema refused, so the resume it documents had no correct input.** A row id is a microsecond epoch — `"1790008926.7639065"`, a string in every row the instance sends — and the tool returned `Number(...)` of it while declaring the parameter an `integer`: three types for one identifier. Handing the value straight back, which is exactly what the description said to do, failed with *"/lastId must be integer"*, and the only value the schema did accept, `1790008926`, re-delivered every row from that second, because `last_id` is a strict greater-than. Tailing logs — the reason to page them — could not be done correctly at all. The cursor is now the string the instance sent, at every step, and the schema accepts it. The description also says what it never did: which stream this reads (the error log that `pos-cli logs` tails), and that `limit` counts from the **oldest** row after the cursor.

* **The admin GUI's Logs page stopped advancing after the first rows arrived**, for the same reason: `logsRequestSchema` declared `lastId` an integer, and the page polls with `logs.at(-1).id`. Every poll after a row had been seen was a 400. Both schemas now describe a row id as the instance writes it, from one shared pattern, so the cursor has a single type wherever it travels — a property the schema's own comment claimed while the two disagreed. The pattern is also what refuses a cursor carrying its own query parameters, which the integer type used to do; `Gateway.logs` encodes the value as well, so that is two guards rather than one.

* **`liquid-exec` rendered its locals as blanks, with `ok: true` and no error.** The endpoint renders in a context where nothing the caller sends is a variable — the whole request body arrives at `context.params` and nowhere else — so the tool's own documented example, `Hello {{ name }}` with `locals: {"name": "world"}`, came back as `Hello ` and reported success. That is worse than a failure: an agent cannot tell a value that did not arrive from Liquid it got wrong from data that is genuinely empty, and an evaluation lost a whole detour to it. Each local is now bound by prefixing the template with `{% assign <name> = context.params.locals.<name> %}`, so `{{ name }}` and `{{ context.params.locals.name }}` both work. Only the path is written into the template, never the value, so nothing has to be escaped and objects, arrays and booleans arrive as themselves; the prefix adds no line, so `Liquid error (line N)` still names the line you wrote; and the template's own `{% assign %}` of the same name still wins. A template sent without locals is passed through untouched. A key that is not a Liquid name, or that would shadow `context`, cannot be bound — it stays reachable at `context.params.locals["..."]` and is named back in `data.unboundLocals` rather than going quiet. `data.error`, which was `null` on every call that got this far, is gone: a field that is always null invites a check that never fires, and a real Liquid failure has always been `ok: false`.

* **The MCP error taxonomy kept the kind and dropped the advice.** `pos-cli` routes every API failure through `lib/ServerError.js`, which has a handler per status and a sentence of advice in each; the MCP server replaced that with a classifier — it had to, since `ServerError` prints and exits — and kept only the classification. So a 413 reached an agent as a status code with no mention of the 50MB limit, a 500 without the fact that platformOS has already been notified, a Partner Portal outage as an anonymous `INSTANCE_UNAVAILABLE`, and an unreachable instance as `fetch failed`, three words that name neither the host nor what happened to it. Each now carries what the CLI tells an operator: `PAYLOAD_TOO_LARGE` with the limit, `PARTNER_PORTAL_UNAVAILABLE` with the instance's own explanation, how long to wait and the fact that the token is not the problem, and a connection failure with the host as a field — its origin only — and whether the name failed to resolve or the host failed to answer. A status pos-cli knows nothing extra about is unchanged: the advice is a field where there is one, not prose on every error.

* **`ENV_NOT_FOUND` did not say which environments there are**, although the resolver had them in hand and `ENV_REQUIRED` has listed them since the guard was added. A model cannot read `.pos`, so a mistyped name cost it an `envs-list` round trip — with a tool it may not have been given. The names are now in the message and in `details.environments`. Where there are none at all, the error carries the command that adds one instead, and says a person has to run it.

* **The server instructions named five of the eight error kinds**, while promising that "the kind says what to do next" — so an agent that received `kind: project` was reading guidance that did not mention it. All eight are described, and the set the instructions have to cover is now derived from the taxonomy itself, so a kind cannot be added without being described. The sentence about `pos-cli env refresh-token` is replaced by a general one about `details.remedy`: one rule covering every remedy, including the two this release adds, instead of one announcement per remedy — and it says that a remedy marked for a person is not one for the agent to run.

* **`deploy-dry-run` reported no deletions for a deploy that deletes.** It read the file report off the *push* response, and the push answers `ready_for_import` with `report: null` — the report, and any validation error, appear on the release a second or two later. So `byCategory` was always empty, the only thing left in it was the asset phase, and the top-level `deleted`/`upserted`/`skipped` were asset numbers under whole-deploy names. An agent that ran the documented pre-flight check before a non-partial deploy was told nothing would be deleted, and then deleted files. It now waits for the release and reports what the release says, with the paths. It also gained a `verdict`: the instance evaluates a dry run and can refuse it outright — a table that still has records cannot be dropped — and that refusal, with the files it names, was previously invisible.

* **An HTML error page no longer travels back in a tool result.** The two this API serves are 1,430 and 2,062 bytes whose entire content is `<title>Aw, Snap!</title>` and `<title>Oops (503)</title>`; an evaluation measured a pair of them at 12% of everything it spent on the server. Only the title is kept now. The bound is applied where the error is turned into a result, so the `/_tests/*` tools — which build their own errors rather than going through the classifier — are covered by it too.

* **`unit-tests-run` and `tests-run-async` said `not_found` when the test runner was not installed.** The runner ships in the `tests` module, so an instance without it has no `/_tests/*` endpoint and answers 404 — the same 404 a test name the runner does not know would get, which sent agents to debug the name. The CLI has always checked (`pos-cli tests run` prints the install command); the MCP tools did not. They now ask which modules are installed, but only after a 404, so a run that works costs nothing extra, and answer `TESTS_MODULE_MISSING` with the command that fixes it.

* **The server instructions promised a refusal that does not always happen.** They said a call that could change an instance is refused when `env` is omitted. It is — but only when `.pos` holds more than one environment; with a single entry there is nothing to choose between and the call proceeds. An agent that read the instructions could believe a forgotten `env` was harmless on a non-partial deploy. The sentence now says which case is which.

* **A `job_id` for a job that does not exist told the agent to retry forever.** A status request for a release, import or export id the instance does not have is answered `503`, not `404` — the same 2,062-byte error page every time, byte-identical for a nonsense id as for a plausible one. So the `404` mapping that was meant to catch this never fired, the failure was classified `unavailable` ("the same call may work later"), and an agent polled a job that would never exist until it gave up on its own. Nothing in the response separates that from a `503` by an instance that is genuinely unwell, so the instance is asked a second question instead: one that answers for itself while refusing one job twice in a row is not unwell, and the job is reported `not_found`. A refused connection, a `503` the instance explains as a Partner Portal outage, and an instance that does not answer the second question are all still reported as retryable — reporting a deploy that was briefly slow as one that never existed is the worse of the two errors.

* **An expired token told the agent nothing it could act on.** Every authenticating tool answers `kind: auth` when a stored instance token is rejected, and `pos-cli env refresh-token` asks for a password and a second factor, so it is deliberately not an MCP tool — leaving the agent with "Unauthorized" and no idea the remedy existed. The error now carries `details.remedy` with the exact command, naming the environment that failed, and says a person has to run it. Only for a credential stored in `.pos`, since that is the only one the command rewrites, and only on `auth`: an instance whose Partner Portal is down cannot judge a token at all, and telling someone to refresh a working one is the mistake that costs them a good credential.

* **An MCP deploy could not deploy assets to an instance with no object storage.** `pos-cli deploy` asks whether the instance can presign an upload before it builds the archive, and sends the assets inside the release when it cannot. `deploy-start` never asked: it always archived without the assets and then uploaded them directly, into a presign that could only fail — so on exactly the instances that need the fallback, the assets never arrived. It now asks the same question in the same order, reports `assets.status: "in_release_archive"` with the reason when it takes the other path, and treats that deploy as finished when its release is in, since there is no second phase to wait for.

* **Two deploys started close together packed over each other.** `deploy-start` wrote `./tmp/release.zip`, `deploy-dry-run` wrote `./tmp/release-dry-run.zip` and the asset upload packed `./tmp/assets.zip`, all fixed names — safe for a CLI that runs one deploy and exits, and not for a server that answers while the deploy is still running and holds those paths for as long as an import takes. Each call now writes into a directory of its own under `tmp/pos-cli-mcp-deploy/`, removed when the deploy is done. **The `archive.path` field is gone** from both tools' results, since there is no longer a file left to open; `archive.fileCount` stays, and `deploy-start` adds `archive.assetsIncluded`.

* **A background asset upload could have its credentials changed underneath it.** `runWithAuth` sets `MARKETPLACE_*` on the process and restores them when it returns, and `deploy-start` wrapped its background upload in it — so for the length of a release import plus a CDN wait, any other tool resolving different credentials changed them under the upload, or cleared them by finishing first. `presignUrl` and `presignDirectory` now take the credentials as an argument, `lib/assets.js` passes the ones its Gateway is already using, and `deploy-start`, `deploy-dry-run`, `uploads-push` and `data-import` no longer set them on the process at all. The environment stays the fallback, so every CLI path is unchanged.

* **Shutting down the HTTP transport now closes the MCP handler, and stops taking new work.** The SDK's handler exposes a `close()` that nothing called, so teardown happened by side effect — `stopHttp` destroyed the streaming responses it had recognised by sniffing the request body, and in-flight per-request servers were left unclosed. It is called now, **after** the drain rather than at the start of one, because it aborts in-flight exchanges and this shutdown exists to let them finish. Separately, `/mcp` refuses a request that arrives once shutdown has begun instead of running a tool nobody is waiting for; a request already being answered is unaffected.

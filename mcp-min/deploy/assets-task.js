 ...
 *
 * It waits for the release to settle first: the CLI only ever sends a manifest for a settled
 * release (`lib/push.js` polls before `deployAssets`), and sending one mid-import is untested
 * against the API. The release id is also what makes the asset phase observable — without it the
 * instance never associates the manifest with the release, so `asset_status` stays empty.
 */
...
/** @returns {Promise<'done'|'failed'>} once the release leaves ready_for_import/in_progress. */
async function waitForRelease(gateway, releaseId, { pollIntervalMs, timeoutMs, wait, now }) {
  ...
  for (;;) {
    ...
    try {
      ...
      // A response with no status in it says nothing about the release.
      state = last === undefined ? 'running' : releaseState(last);
      if (state !== 'running') return state;
    } catch (err) {
      // Giving up here means the assets are never uploaded at all.
